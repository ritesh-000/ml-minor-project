...
#         # copy the wt and biass from policy=>target
...
#             else:
#                with torch.no_grad():
#                  action = policy_dqn(state.unsqueeze(dim =0)).squeeze().argmax() #bexploit
...
class Agent:
    ...
    def run(self, is_training=True, render=False):
        ...
        for episode in itertools.count():
            ...
            print(f"Episode {episode+1} | Reward = {episode_reward} | Epsilon = {epsilon if is_training else 'N/A'}")
